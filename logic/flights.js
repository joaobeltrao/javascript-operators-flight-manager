function Flights() {
    function calculateNumberOfFlights(passengers, capacity) {
        if (passengers<1 || Math.floor(passengers)!=passengers) throw new Error ("The number of passengers must be a positive integer value");
        if (capacity<1 || Math.floor(capacity)!= capacity) throw new Error ("The capacity of the flight must be a positive integer value");
    ...
    function checkAircraftRevision(distanceLimit, distanceArr) {
        ...
        if (totalDistance > distanceLimit) throw new Error ("The maximum allowed flight distance should be exceeded");
    }
